...
- Every rendered line is bounded to the terminal width — that is a tested invariant (`every_*_fits_the_terminal_width` in `src/output.rs`), not a nicety, so a new panel line must wrap (`wrap_plain`/`wrap_styled`) or truncate rather than soft-wrap. The width fallback chain (`terminal_size` → `COLUMNS` → 80) has one implementation, `progress::terminal_width`; `output::terminal_width` is its presentation-side name. Narrow terminals degrade instead of wrapping: the workspace table stacks below 84 columns, the clean review below `REVIEW_COLUMNAR_MIN_W`.
- Which panel a targeted `offcut clean <PROJECT_PATH>` ends on is `output::blocks_cleaning`, not "is it cleanable": a blocking tree state gets `format_blocked_project` (refuse + the action that unblocks it), `Clean` gets `format_nothing_to_reclaim`, and a `Cleanable` project whose inspection failed gets the plain summary line (`main::targeted_outcome_panel`). Never route `Clean` to the refusal — it tells a committed, pushed project to commit and push.
