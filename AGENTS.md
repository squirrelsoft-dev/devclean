...
- `devclean list` is the only observable product command today; `devclean ignore` and `devclean safelist` are debug hooks for the matcher and the safe-to-delete catalog. All three are documented in `README.md`; discovery/classification/cleaning are separate issues.
