...
- `--force` and `--dry-run` may be combined (the #8 flow removed the old clap `conflicts_with`): `--force --dry-run` previews the force run. Precedence lives in one place each — `dry_run` alone gates execution, `force` alone gates prompting/auto-approval — do not add further runtime precedence logic.
- `devclean list` is the observable config-printing command; `devclean discovery` finds projects (issue #5 — see `src/discovery.rs`); `devclean classification` classifies them (issue #6 — see `src/classify.rs`); `devclean ignore` and `devclean safelist` are debug hooks for the matcher and the safe-to-delete catalog; `devclean clean` is the interactive cleaning flow (issues #7/#8 — see `src/clean.rs` and `src/interactive.rs`) and **deletes files** once approved or under `--force`. All six are documented in `README.md`.
...
- Public API seam driven by the interactive flow (#8, `src/interactive.rs`): `clean(project, ignore_set, safe_set, approved, force, dry_run)` and `dry_run(...)`; `build_exclusions` is the exclusion-list builder. The `devclean clean` CLI hook is now **destructive**: it deletes via `clean()` only for a project the user approved (or under `--force`), and never under `--dry-run`. The flow shows each project's item report *before* any prompt (approval is never sight-unseen); prompts and the pre-approval report go to stderr, the final outcome report to stdout.
