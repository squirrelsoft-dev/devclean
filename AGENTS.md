...
- Classification, sizing, and cleaning phases: `main.rs::classify_projects` is the single shared classify-with-progress loop (used by `run_listing`, `run_classification`, `run_cleaning`) rendering `classifying N/M`; do not re-inline per-caller copies. `run_listing`'s sizing loop renders `sizing N/M` around each cleanable project's `dry_run` + `compute_reclaimable_size` (single-pass — raw bytes stored once, aggregate is the sum of those bytes, no second walk). The clean execution loop in `run_cleaning` renders `cleaning N/M` around the actual `clean::clean` call and `clear()`s before each interleaved per-project output block.
