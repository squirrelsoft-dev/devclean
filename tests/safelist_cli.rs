//! Integration tests for the `offcut safelist` debug subcommand, which is
...
/// happens to have a `offcut/config.toml` would silently change what these
/// tests assert — and a malformed one would fail them for unrelated reasons.
fn isolated_home() -> &'static Path {
    ISOLATED_HOME.get_or_init(|| {
        let mut d = std::env::temp_dir();
        d.push(format!("offcut-safelist-cli-home-{}", std::process::id()));
...
fn offcut() -> Command {
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_offcut"));
...
/// Run `offcut safelist <path>` inside `cwd`, returning the trimmed stdout.
...
fn safelist_subcommand_with_default_config_falls_back_on_missing_file() {
    // The isolated home has no `offcut/config.toml`, so this genuinely
...
fn safelist_subcommand_with_malformed_pattern_reports_an_error() {
    ...
    // surface as `offcut: <error>` + a non-zero exit like every other config
    // problem — never as a panic with a backtrace.
    let root = unique_dir("malformed");
    let cfg_path = root.join("offcut.toml");
    ...
    assert!(!ok, "{out}");
    assert!(out.contains("offcut: "), "{out}");
