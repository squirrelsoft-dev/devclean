...
///
/// The template body is generated from `Config::default()` via
/// `toml::to_string_pretty`, then a comment block is prepended and the
/// `workspace_roots` line is replaced with the user's path. The resulting
/// file parses back to a valid `Config` (round-trip).
